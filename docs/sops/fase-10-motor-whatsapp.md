...
| **F10b — Núcleo vs mock (S10–S18)** | 🟢 CÓDIGO LISTO | **B1 ✅** (`8163838`) · **B2 ✅** (`26e0c90`, seed aplicado) · **B3 ✅** (S18 panel + harness) · pendiente: **smoke en vivo del golden path** (entorno de Iván: Docker + ANTHROPIC_API_KEY) |
...
| **F10c — Canal real gated (S19–S22)** | 🟢 CÓDIGO LISTO | S19/S20/S21/S22 ✅ (todo codeado + gated; migr 018 aplicada). Falta go-live (cuentas reales + deploy) y S23 |
| S19 — YCloud real + welcome-template | ✅ | `send-welcome-template.ts` (YCloud, sin GHL: ycloudSendTemplate + persiste role=agent + conversation_source='bienvenida'/fase 1) + `YCLOUD_BUSINESS_PHONE` env + wiring en outbound-sender. Gated (sin trigger/go-live) |
| S20 — Cal.com + webhook + migr 018 | ✅ | `lib/calcom-client.ts` (GET /v2/slots + POST /v2/bookings) + rama calcom en consultar-disponibilidad/agendar-visita (espejo `calendar_appointments` best-effort) + `calcom-provider.ts` + `appointment-matcher`/`appointment-applier` + `routes/webhook-calcom.ts` (HMAC) + parser rawBody. **Migr 018 APLICADA** (`ALTER TYPE`+regen tipos) · 42/42. Tests HTTP-mock + fake. Gated |
| S21 — seguridad dura | ✅ | `lib/calcom-verify.ts` (HMAC x-cal-signature + timingSafeEqual) + `lib/assert-url.ts` (assertHttpsUrl) + `CALCOM_WEBHOOK_VERIFY_MODE` enforce/warn/disabled en webhook-calcom. Tests de firma + url. (cron sigue OFF) |
...
| **S23 — Cierre** | 🟢 PARCIAL | ✅ verificación (typecheck/build/100 tests/42-42/0 service-role panel) · ✅ push `checkpoint/fase-10` · ✅ **panel promovido a prod** (vega-hogar-panel `g5v3yw87t` Ready; /login 200, /conversations 307) · ✅ memoria. PENDIENTE: smoke en vivo del motor (entorno Iván) + go-live externo. (Nota: el `vercel --prod` desde raíz creó un proyecto suelto `comercial_inmobiliaria` — borrar del dashboard) |
