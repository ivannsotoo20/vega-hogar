...
- 🔴 **No existe la ruta de webhook YCloud entrante real** (`POST /webhooks/ycloud/:tenant_token`). Solo el mock.
  Piezas listas y testeadas (`verifyYCloudSignature`, `parseYCloudInbound`, driver) → falta **la ruta + registro**
  (~200 ln, espejo de `webhook-mock-whatsapp.ts` + `webhook-calcom.ts`). **Único bloqueante de código.**
...
| **G2** — 🔴 Webhook YCloud entrante real (código) | ⬜ PENDIENTE | `routes/webhook-ycloud.ts` + registro + E.164 + tests |
...
- **Supabase** (con OK; `scripts/seed-integration-accounts.mjs` cifra + inserta, revisar RETURNING):
...
Redis en modo usuario dentro de WSL + el motor por `tsx`. Requisitos: `pnpm install` hecho + `.env.local` con
`ANTHROPIC_API_KEY`, `MOTOR_CRON_ENABLED=true`, `CREDENTIALS_ENCRYPTION_KEY` (64 hex), `INTERNAL_STATS_TOKEN` (≥16).

Redis userspace ya preparado en WSL (`~/rdl/` extraído sin sudo vía `apt-get download` + `dpkg -x`; lanzador
`~/rdl/run-redis.sh`). Para levantar todo:
```powershell
# 1) Redis (dejar la terminal abierta):
wsl -d Ubuntu bash /home/sotobivan/rdl/run-redis.sh
